...
void OnTick() {
   ...
   // --- 1. SKANOWANIE POZYCJI (Zliczanie zysku netto) ---
   ...
   if(Global_MoneySL_Active) {
      ...
      if(activeMarketPositions > 0) {
         ...
         if(Global_MoneySL_Value < 0) {
            // TRYB STOP LOSS (Strata): Zamyka gdy zysk spadnie poniżej -100 (np. jest -110)
            if(totalNetProfit <= Global_MoneySL_Value) triggerClose = true;
         } 
         else if(Global_MoneySL_Value > 0) {
            // TRYB ZYSKU (Zależny od tego, co było na starcie)
            if(Global_InitialProfit < Global_MoneySL_Value) {
               // A. TARGET: Czekamy na WZROST zysku do celu (np. mam +10, czekam na +100)
               if(totalNetProfit >= Global_MoneySL_Value) triggerClose = true;
            } else {
               // B. TRAILING: Czekamy na SPADEK zysku do progu (np. mam +200, czekam na cofkę do +100)
               if(totalNetProfit <= Global_MoneySL_Value) triggerClose = true;
            }
         }

         if(triggerClose) {
             Print("Money SL/TP Triggered! Profit: ", totalNetProfit, " Initial: ", Global_InitialProfit, " Target: ", Global_MoneySL_Value);
             CloseAll();
             DeleteAllPending();
             ResetAllModes();
             BasketStarted = false;
...
void OnChartEvent(const int id, const long &lparam, const double &dparam, const string &sparam) {
   ...
   if(sparam == "BTN_MONEY_SL") {
          ...
          // SKANUJEMY ZYSK W CHWILI KLIKNIĘCIA
